...
// app.put('/api/empresas/:id', async (req, res) => {
//   const { id } = req.params;
//   const { nome, promptIA, telefone, botAtivo, msgBoasVindas, timeoutHumanoMinutos } = req.body;
...
//     const empresaAtualizada = await Empresa.findByIdAndUpdate(
//       id,
//       { nome, promptIA, telefone, botAtivo, msgBoasVindas, timeoutHumanoMinutos },
//       { new: true, runValidators: true }
//     );
...
app.post('/api/empresas', async (req, res) => {
    // MUDANÇA: Adicionado msgBoasVindas, timeoutHumanoMinutos, msgFechado e horariosSemana
    const { 
    ...
    try {
      ...
      const novaEmpresa = new Empresa({ 
          ...
          msgBoasVindas, timeoutHumanoMinutos, msgFechado, horariosSemana // Inclui novos campos
      });
      await novaEmpresa.save(); // Salva para ter o _id
...
app.put('/api/empresas/:id', async (req, res) => {
  const { id } = req.params;
  // MUDANÇA: Adicionar todos os novos campos editáveis
  const { 
      nome, promptIA, telefone, botAtivo, 
      msgBoasVindas, timeoutHumanoMinutos, msgFechado, horariosSemana 
  } = req.body;
  ...
  try {
    ...
    // MUDANÇA: Incluir todos os campos no objeto de atualização
    const empresaAtualizada = await Empresa.findByIdAndUpdate(
      id,
      { 
          nome, promptIA, telefone, botAtivo, 
